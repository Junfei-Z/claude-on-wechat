...
import { bodyFromItemList, findMediaItems, setContextToken } from "./weixin/inbound.js";
  ...
  while (!abortSignal?.aborted) {
    try {
      ...
      for (const msg of msgs) {
        ...
        // Extract and download media items (images, files, videos)
        const mediaItems = findMediaItems(msg.item_list);
        const mediaPaths: string[] = [];

        for (const mediaItem of mediaItems) {
          try {
            const tmpDir = path.join(config.dataDir, "media");
            const result = await downloadMediaFromItem(mediaItem, {
              cdnBaseUrl: config.weixinCdnBaseUrl,
              tmpDir,
              label: `msg-${fromUserId}`,
            });
            if (result.decryptedPicPath) mediaPaths.push(result.decryptedPicPath);
            if (result.decryptedFilePath) mediaPaths.push(result.decryptedFilePath);
            if (result.decryptedVideoPath) mediaPaths.push(result.decryptedVideoPath);
            if (result.decryptedVoicePath) mediaPaths.push(result.decryptedVoicePath);
          } catch (err) {
        ...
        if (mediaPaths.length > 0) {
          contentParts.push(`[附件已保存到本地]\n${mediaPaths.map(p => `文件路径: ${p}`).join("\n")}`);
        }
        const content = contentParts.join("\n\n");

        logger.info(`inbound: from=${fromUserId} text="${(text || "").substring(0, 100)}" media=${mediaPaths.length}`);
